...
# ╔═╡ d9a12427-cd74-4c3a-920d-245fde96ed88
md"""To improve this language generation even more, we can consider the frequencies of letter _combinations_, which are called "transition frequencies". We can visualize this using the previous sample of English language by creating a **transition frequency matrix** below:"""
...
dorian = let
	raw_text = read(download("https://ia801600.us.archive.org/2/items/pictureofdoriang00wildiala/pictureofdoriang00wildiala_djvu.txt"), String)
	
	first_words = "THE studio"
	last_words = "THE END"
...
imresize(download("https://www.science-sparks.com/wp-content/uploads/2022/04/Hookes-LAw-1-scaled.jpeg") |> load, ratio=0.4)

# ╔═╡ 05e4fd4f-983b-4fd0-a58c-14b40a2e811b
md"""**Why neural networks?**
...
function completion_cache(grams)
	cache = Dict()
	# iterate through each ngram in the array of ngrams
	n = length(grams[1])
	for original_n_gram in grams
		key = original_n_gram[1:n-1]
		if haskey(cache,key)
			push!(cache[key], original_n_gram[n])
		else
			cache[key] = [original_n_gram[n]]
		end	
	end
...
# ╔═╡ d7526d80-e6f3-45b7-ab86-aeccf89bcc6d
...
# ╔═╡ f95d3a79-1b33-4c5c-98ff-dd3c2768fe23
...
# ╔═╡ d8af1ea2-2745-41b8-bdf7-bbe8b77524a1
unaccent(str) = Unicode.normalize(str, stripmark=true)

# ╔═╡ 70717bb4-a46b-4dd7-8f31-9934e141fcea
function ngrams_circular(words, n)
	ngrams([words..., words[1:n-1]...], n)
end

# ╔═╡ cb34b064-694c-426c-bd88-430b2375d758
...
# ╔═╡ 0555b1fc-9f40-446a-aff7-ca0d0459dc93
...
# ╔═╡ 3f9b1e66-438c-47c4-8529-b35b4b79cc3c
...
# ╔═╡ d913be70-54d1-4a41-ab8b-28beb8c14f44
alphabet = ['a':'z' ; ' '] 

# ╔═╡ b93be3fc-4f14-461b-95b5-b80904ad7881
...
# ╔═╡ 31d1cd3b-1b1f-4214-93b1-728b34a3b0a3
function isinalphabet(character)
	character ∈ alphabet
end

# ╔═╡ 08ea3732-b87d-4c9c-ad84-dad0894674ce
...
# ╔═╡ 54bf1f40-c2e9-4cf0-8e94-588a5b4022f5
...
# ╔═╡ 2fdf80b3-07f5-422d-8a2e-5d594f903fe8
...
# ╔═╡ 088c5a9f-fc88-43fd-b2f0-6f2b3262dd34
function clean(text)
	cleaned_string = filter(isinalphabet, (unaccent(lowercase(text))))
end

# ╔═╡ e7ed64dd-2750-4035-88ac-5a8e776e8844
function rand_sample(frequencies)
	x = rand()
	findfirst(z -> z >= x, cumsum(frequencies ./ sum(frequencies)))
end

# ╔═╡ 1a9db8c0-1de8-4404-b11d-444f6bc561bc
function rand_sample_letter(frequencies)
	alphabet[rand_sample(frequencies)]
end

# ╔═╡ 48507303-451d-4c1e-8f87-b2cabc8f848c
...
The word forest derives from the Old French forest, denoting "forest, vast expanse covered by trees"; forest was first introduced into English as the word denoting wild land set aside for hunting[14] without the necessity in definition of having trees on the land.[15] Possibly a borrowing, probably via Frankish or Old High German, of the Medieval Latin foresta, denoting "open wood", Carolingian scribes first used foresta in the Capitularies of Charlemagne specifically to denote the royal hunting grounds of the King. The word was not endemic to Romance languages, e. g. native words for forest in the Romance languages derived from the Latin silva, which denoted "forest" and "wood(land)" (confer the English sylva and sylvan); confer the Italian, Spanish, and Portuguese selva; and the Old French selve, and cognates in Romance languages, e. g. the Italian foresta, Spanish and Portuguese floresta, etc., are all ultimately derivations of the French word. 
""",
	Spanish =  """
...
sample_text(transition_frequencies(clean(first_sample)), 400) |> Quote

# ╔═╡ 2d0eab72-fe5e-4b7c-847d-75fc32c34ebc
sample_freqs = letter_frequencies(first_sample)

# ╔═╡ 0110e827-f06f-47f4-a49f-e3fee2c76b76
...
julia_version = "1.8.4"
manifest_format = "2.0"
project_hash = "cb99863c1959117b827d836f37a8092797659c92"
